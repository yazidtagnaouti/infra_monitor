import sys, os
import time  # was going to use for timing, didn't
...
def run_pipeline(data_str, _key):  # workaround: _key busts cache when groq key changes
...
def _collect_anomalies(records):
    """Rebuild anomaly rows for the UI table/charts."""
    out = []
    for r in records:
        for metric, levels in THRESHOLDS.items():
            ...
            if val is None:
                continue
            if val >= levels["crit"]:
                out.append({"Timestamp": r["timestamp"], "Métrique": metric, "Valeur": val, "Sévérité": "critical"})
            elif val >= levels["warn"]:
                out.append({"Timestamp": r["timestamp"], "Métrique": metric, "Valeur": val, "Sévérité": "warning"})
...
with tab2:
    all_anomalies = _collect_anomalies(records)

    bar_rows = [{"Métrique": a["Métrique"], "Sévérité": SEVERITY_LABELS[a["Sévérité"]], "count": 1} for a in all_anomalies]
    df_bar = (
    ...
    with c1:
        ...
        fig_b.update_layout(barmode="stack", legend_title="Sévérité", coloraxis_showscale=False, plot_bgcolor="rgba(0,0,0,0)")
    ...
    if not df_anom.empty:
        df_anom = df_anom.assign(
            ...
            Valeur=df_anom["Valeur"].astype(str),  # mixed nums + service status strings
        )
        ...
        def _severity_style(val):
            ...
            if not color:
                return ""
            return "background-color: {}; color: #FFFFFF; font-weight: 600".format(color)
...
with tab3:
    ...
    if llm:
        st.caption("Recommandations via Groq (LLM)")
    else:
        st.caption("Règles Python — ajoutez une clé Groq (config.py ou sidebar) pour l'enrichissement IA")

    for r in recos:
        title = "[{prio}] [{rid}] {t}".format(
            prio=r.get("priority"), rid=r.get("id"), t=r.get("title")
        )
        with st.expander(title, expanded=(r.get("priority") == "Critique")):
            c1, c2, c3 = st.columns(3)
            c1.markdown("**Priorité** : {}".format(r.get("priority")))
            c2.markdown("**Effort** : {}".format(r.get("effort")))
            c3.markdown("**Impact** : {}".format(r.get("impact")))
...
with tab4:
    ...
    for i, (svc, stats) in enumerate(services.items()):
        ...
        with cols[i]:
            st.markdown(
                f"<p style='text-align:center;font-weight:600;font-size:1.1rem;margin:0 0 0.5rem;'>{svc}</p>",
                unsafe_allow_html=True,
            )
            st.plotly_chart(fig_g, width="stretch")
            st.markdown(
                f"<p style='text-align:center;font-size:1.75rem;font-weight:700;margin:0.25rem 0 0.5rem;'>{pct}%</p>",
                unsafe_allow_html=True,
            )
            st.markdown(
                "<p style='text-align:center;margin:0.25rem 0 0;'>{online} online · {deg} dégradés · {off} offline</p>".format(
                    online=stats["online"], deg=stats["degraded"], off=stats["offline"]
                ),
                unsafe_allow_html=True,
            )
...
with tab5:
    ...
    with c2:
        st.download_button(
            ...
            file_name="rapport_{}.json".format(ts),
            mime="application/json",
        )
